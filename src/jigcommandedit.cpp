...
#include "ui_jigcommandedit.h"

#include <QDebug>

JigCommandEdit::JigCommandEdit(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::JigCommandEdit)
{
    ui->setupUi(this);
...
JigCommandEdit::~JigCommandEdit()
{
    delete ui;
}

void JigCommandEdit::loadCommand(int index, JigCommand *cmd)
{
    this->index = index;
    this->command = cmd;
    ...
    ui->lineEdit_codeName->setText(command->getCodeName());
...
void JigCommandEdit::on_buttonBox_accepted()
{
    ...
    command->setCodeName(ui->lineEdit_codeName->text());
    ...
    command->setState(JigCommand::pending);
}

void JigCommandEdit::setInterfaces(QStringList interfaces)
{
    this->interfaces->setStringList(interfaces);
}

void JigCommandEdit::setCommands(QStringList commands)
{
    this->commands->setStringList(commands);
}

void JigCommandEdit::on_doubleSpinBox_mean_valueChanged(double arg1)
{
    qDebug() << arg1;
...
void JigCommandEdit::on_doubleSpinBox_deviation_valueChanged(double arg1)
{
    qDebug() << arg1;
...
void JigCommandEdit::on_checkBox_formula_toggled(bool checked)
{
